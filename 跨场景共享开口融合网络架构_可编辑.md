...
| 相对深度响应 | `log(D_ext / D_enc)` planned | `D_enc`, `D_ext` | `relative_delta: B x 256` | 待实现 |
| 开口外延深度估计 | `G_open = Delta * P_open` planned | `Delta`, `P_open` | `G_open: B x 256` | 待实现 |
...
| 训练版 opening head | `OpeningHead` planned | `concat(F_seq, Delta, relative_delta, corner_heat)` | `P_open`, `G_open`, width | 待实现 |
...
实现方法：第一版用规则实现，计算 `Delta = relu(D_ext - D_enc)` 和相对响应 `log(D_ext / D_enc)`，对 1D 信号做平滑和阈值分割，得到连续 token 区间；再根据 `wall_token_assignment` 把 token 区间映射到具体墙段上的 `startRatio/endRatio`。训练版可把 `concat(F_seq, Delta, relative_delta, corner_heat)` 输入轻量 `Conv1D/MLP`，输出 `passability_heat`、`G_open` 和候选宽度。
...
```text
OpeningHead(concat(F_seq, Delta, relative_delta, corner_heat)) -> passability_heat, G_open, width
```
...
| M4 1D 开口响应与外延深度 MVP | `utils/cross_scene_estimator.py::extract_opening_candidates` |
